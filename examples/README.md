...
- [`internal`](internal) — the domain code both of the above share
  ...
  and validation schemas, the paginated list shape, the cache driver switch)
  and `internal/app` (logging setup, a query-param clamp helper). Neither
  subpackage is framework-specific, and Go's `internal/` visibility rule
  keeps both scoped to `examples/...` — they aren't part of this module's
  public API.
...
Both wire together every non-stub package in this module:

| Package                                          | Used for                                            |
| ------------------------------------------------- | ---------------------------------------------------- |
| [`env`](../env)                                    | reading `PORT`/`DB_URL`/`CACHE_DRIVER`/etc.          |
...
`storage` isn't included: it's still an empty stub package (see the root
[README](../README.md#status)), nothing to wire up yet.
